  ...
  const rawSteps: Omit<DemoStep, 'animationSequence'>[] = [
    {
      ...
      chatSequence: [
        ...
        { id: nextId(), type: 'ai', content: `[질문 1/2] ${s.questions[0].q}\n\n${s.questions[0].options.join('\n')}\n\n[Answer]:` },
        ...
        { id: nextId(), type: 'ai', content: `[질문 2/2] ${s.questions[1].q}\n\n${s.questions[1].options.join('\n')}\n\n[Answer]:` },
        ...
        { id: nextId(), type: 'ai', content: `총 ${s.mainFeatures.length + 5}건의 요구사항이 정의되었습니다.\n\n[질문 3/3] MVP 범위를 결정해주세요.\n\nA) 핵심 기능만 (${s.mainFeatures[0]} + 인증)\n   - 출시: 4주, 리스크: 낮음\nB) 핵심 + 부가 기능 (${s.mainFeatures.slice(0, 3).join(' + ')})\n   - 출시: 8주, 리스크: 중간\nC) 전체 기능 (${s.mainFeatures.length}건 모두 포함)\n   - 출시: 12주, 리스크: 높음\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: `B) 핵심 + 부가 기능` },
      ],
    },
    {
      phase: 'INCEPTION',
      ...
      chatSequence: [
        ...
        { id: nextId(), type: 'ai', content: `[질문 1/1] ${s.questions[2].q}\n\n${s.questions[2].options.join('\n')}\n\n[Answer]:` },
        ...
        { id: nextId(), type: 'ai', content: `스토리 우선순위를 정리합니다.\n\n[질문 2/2] Sprint 1에 포함할 스토리를 선택해주세요.\n\nA) ${user1} 중심 (${s.userStories[0].stories[0]})\n   - 사용자 가치 우선, 빠른 피드백 확보\nB) ${user2} 중심 (${s.userStories[1].stories[0]})\n   - 운영 안정성 우선, 모니터링 기반 확보\nC) 양쪽 균형 (${user1} 1건 + ${user2} 1건)\n   - 균형 잡힌 진행, 양측 피드백 동시 확보\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: `C) 양쪽 균형` },
      ],
    },
    {
      phase: 'INCEPTION',
      ...
      chatSequence: [
        ...
        { id: nextId(), type: 'ai', content: `[질문 1/2] 상태 관리 전략을 선택해주세요.\n\nA) React Context + useReducer\n   - 가볍고 빠른 구현, 소규모 앱에 적합\nB) Redux Toolkit + RTK Query\n   - 예측 가능한 상태, 대규모 앱에 적합\nC) Zustand + React Query\n   - 최소 보일러플레이트, 서버 상태 분리\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'C) Zustand + React Query' },
        ...
        { id: nextId(), type: 'ai', content: `[질문 2/2] API 인증 방식을 선택해주세요.\n\nA) JWT + Refresh Token\n   - 무상태, 수평 확장 용이\nB) Session 기반 (Redis)\n   - 즉시 무효화 가능, 서버 부하 있음\nC) OAuth 2.0 + OIDC\n   - 소셜 로그인 연동, 표준 프로토콜\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'A) JWT + Refresh Token' },
      ],
    },
    {
      phase: 'CONSTRUCTION',
      ...
      chatSequence: [
        ...
        { id: nextId(), type: 'ai', content: `[질문 1/1] NFR 우선순위를 확인합니다.\n\nA) 성능 최우선 (P95 < 200ms, 동시 5,000명)\nB) 보안 최우선 (Zero Trust, 데이터 암호화)\nC) 확장성 최우선 (자동 스케일링, 멀티 리전)\nD) 균형 잡힌 설계 (모든 영역 표준 수준)\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'D) 균형 잡힌 설계' },
        ...
        { id: nextId(), type: 'ai', content: `[질문 2/2] 캐싱 전략을 선택해주세요.\n\nA) CDN + 브라우저 캐시 (정적 자산 중심)\n   - 구현 간단, 비용 낮음\nB) Redis 캐시 + CDN (API 응답 캐싱 포함)\n   - DB 부하 감소, 응답 속도 향상\nC) 멀티 레이어 캐시 (CDN + Redis + 인메모리)\n   - 최고 성능, 복잡도 높음\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'B) Redis 캐시 + CDN' },
      ],
    },
    {
      phase: 'CONSTRUCTION',
      ...
      chatSequence: [
        ...
        { id: nextId(), type: 'ai', content: `코드 생성 방식을 선택해주세요.\n\n[질문 1/2] Code Generation 방식:\n\nA) TDD 방식 (Test-Driven Development)\n   - 시간/토큰: 1.5~2배 소요\n   - 품질: 높음 (기능 누락 방지, 테스트 우선 설계)\n   - 권장: 복잡한 비즈니스 로직, 장기 유지보수 프로젝트\n\nB) 일반 방식 (Standard)\n   - 시간/토큰: 기준\n   - 품질: 표준\n   - 권장: 간단한 프로토타입, 일회성 스크립트\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'A) TDD 방식' },
        { id: nextId(), type: 'ai', content: `TDD 방식으로 코드를 생성합니다.\n\n생성 순서:\n1. Contract/Interface 스켈레톤\n2. 테스트 코드 (RED)\n3. 구현 코드 (GREEN)\n4. 리팩토링 (REFACTOR)` },
        { id: nextId(), type: 'ai', content: `[질문 2/3] 테스트 커버리지 목표를 선택해주세요.\n\nsrc/\n  app/ (라우팅)\n  components/ (${s.mainFeatures[0]}, ${s.mainFeatures[1]})\n  hooks/ (useAuth, useApi)\n  services/ (api.ts)\n  types/ (index.ts)\n\nA) 핵심 경로만 (커버리지 60%, 빠른 피드백)\nB) 주요 기능 전체 (커버리지 80%, 균형)\nC) 엣지케이스 포함 (커버리지 95%, 높은 안정성)\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'B) 주요 기능 전체 (커버리지 80%)' },
        ...
        { id: nextId(), type: 'ai', content: `테스트 통과를 위한 구현 코드를 작성합니다 (GREEN 단계)...\n\n- 단위 테스트: 각 컴포넌트별 렌더링 + 인터랙션\n- API 테스트: 엔드포인트별 요청/응답 검증\n- 통합 테스트: 사용자 시나리오 기반 E2E\n\n총 ${s.mainFeatures.length * 3}개 테스트 케이스 ALL PASSED` },
        { id: nextId(), type: 'ai', content: `리팩토링을 수행합니다 (REFACTOR 단계)...\n\n- 코드 품질: ESLint + Prettier 적용\n- 타입 안전성: TypeScript strict 모드\n- 접근성: ARIA 속성 + 키보드 네비게이션\n- 성능: React.memo + useMemo 최적화` },
        { id: nextId(), type: 'ai', content: `[질문 3/3] 에러 핸들링 전략을 선택해주세요.\n\n${s.mainFeatures.length}개 컴포넌트, ${s.apiEndpoints.length}개 API 라우트가 생성되었습니다.\n\nA) 글로벌 Error Boundary + Toast 알림\n   - 사용자 친화적, 구현 간단\nB) 에러 코드 체계 + Retry 로직 + Fallback UI\n   - 세밀한 제어, 복구 가능성 높음\nC) Circuit Breaker + 에러 리포팅 (Sentry)\n   - 장애 전파 차단, 실시간 모니터링\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'B) 에러 코드 체계 + Retry 로직' },
      ],
    },
    {
      phase: 'OPERATIONS',
      ...
      chatSequence: [
        ...
        { id: nextId(), type: 'ai', content: `[질문 1/1] 네트워크 아키텍처를 확인합니다.\n\nA) 멀티 AZ (고가용성, 비용 높음)\nB) 단일 AZ (비용 최적화, 가용성 낮음)\nC) 멀티 리전 (글로벌 서비스, 비용 최고)\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'A) 멀티 AZ' },
        ...
        { id: nextId(), type: 'ai', content: `[질문 2/2] 비용 최적화 전략을 선택해주세요.\n\n${s.awsServices.length}개 AWS 서비스가 구성되었습니다.\n\nA) Savings Plans (1년 약정, 30% 절감)\n   - 안정적 워크로드에 적합\nB) Spot Instance + Auto Scaling (최대 70% 절감)\n   - 내결함성 워크로드에 적합\nC) 온디맨드 유지 (약정 없음)\n   - 유연성 최대, 비용 최고\n\n[Answer]:` },
        { id: nextId(), type: 'user', content: 'A) Savings Plans' },
      ],
    },
    {
      phase: 'OPERATIONS',
